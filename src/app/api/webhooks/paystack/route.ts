...
export async function POST(req: NextRequest) {
  ...
  let payload: { event: string; data: { reference: string; status: string; amount: number; metadata?: { donation_id?: string } } };
  try {
  ...
  if (payload.event === "charge.success" && payload.data.status === "success") {
    const donationId =
      payload.data.metadata?.donation_id ??
      payload.data.reference.replace("donation-", "");

    if (donationId) {
      await markDonationComplete(donationId, payload.data.reference);
    }
  }
